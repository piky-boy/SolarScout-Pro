...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const systemPrompt = `You are a senior B2B sales copywriter for a European commercial solar installation firm (Spain, Portugal, Romania, Albania). You write concise, high-conversion outreach that leads with concrete numbers and respects the reader's time. NEVER invent data — only use the facts provided. Round euro amounts to the nearest hundred. Write in ${languageLabel} (ISO code: ${language}). Tone: ${tone}. NEVER use emojis in the email or call script; a single subtle emoji in the WhatsApp message is OK. Use [Your name] and [Your company] as placeholders for the sender; never make them up.`
